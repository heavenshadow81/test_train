...
public class SGT_RandomiseDebris : SGT_MonoBehaviour
{
	...
	private void ResetAsteroid()
	{
		if (hasBeenReset == false)
		{
			...
			if (GetComponent<Rigidbody>() != null && massScale > 0.0f)
			{
				GetComponent<Rigidbody>().mass = massScale * ((4.0f / 3.0f) * Mathf.PI * (scale * scale * scale));
			}
		}
		...
		{
			GetComponent<Rigidbody>().linearVelocity        = Random.insideUnitSphere * maxSpeed;
			GetComponent<Rigidbody>().angularVelocity = new Vector3(RSpin, RSpin, RSpin);
		}
	}
