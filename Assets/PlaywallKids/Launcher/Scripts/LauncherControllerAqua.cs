...
namespace ML.PlaywallKids.Launcher
{
    ...
    /// 플레이월키스 초기화 및 실행 (진입부)
    /// </summary>
    public class LauncherControllerAqua : MonoBehaviour
    {
        ...
        IEnumerator Start()
        {
            _mc = System.Net.NetworkInformation.NetworkInterface.GetAllNetworkInterfaces()[0].GetPhysicalAddress().ToString();
            print(_mc);
...
#endif
            if (!(_mc == "200DB0436674"|| _mc == "8C89A5E6E02E"))
            {
                messageText.text = "검증되지 않은 PC입니다.";
                yield return new WaitForSeconds(1);
                messageText.text = "종료합니다";
                yield return new WaitForSeconds(1);
                Application.Quit();
                yield return null;
                yield break;
