...
public class CsTestPose : MonoBehaviour {
    public TextAsset rbmText;

    public CsMotionControl motionControl;
    AP3 ap3;
    AP2 ap2;
    RBMFile rbmFile;
    ...
    const float MILLS_TO_TIME = 0.00104f;
    const int kJointCount = 16;

    // 현재 조인트 값
	float[] joints = new float[16];

    // 기본 동작을 위한 초기 조인트 값
    float[] baseJointValues = new float[kJointCount] {
        125,
        179,
        199,
        88,
    ...
    float[] jointOffsetRotations = new float[kJointCount] {
        0.0f,
        -30.0f, 
        64.0f,
        34.0f,
    ...
    // 파일에서 설정된 초기 조인트 값
    float[] initialSetJointValues = new float[kJointCount];

    // used in SendJoint()
    float[] innerJoints = new float[kJointCount];
    ...
    void Start()
    {
        ...
        {
            motionControl = GetComponent<CsMotionControl>();
            if (motionControl == null)
            {
                motionControl = FindObjectOfType<CsMotionControl>();
            }
        }
        ...
        for (int i = 0; i < rbmFile.Header.wCKCount && i < kJointCount; i++)
        {
    ...
    void OpenAP(string fn)
    {
        ...
        else
        {
            ...
            /*
            if(ap3.saveWAVFile(Application.persistentDataPath + "/" + filenameWithoutExtension + ".wav")) 
            {
                StartCoroutine(OpenWAVFromAP3(Application.persistentDataPath + "/" + filenameWithoutExtension + ".wav"));
            }
             * */
        }
    }
        ...
        if (playing)
        {
            ...
            else if (rbmFile != null)
            {
                ...
                foreach (var wck in scene2.wCKInfoList)
                {
                    ...
                    float to = (float)wck.DPOS;
                    float dt = (scene2.SceneTime * MILLS_TO_TIME - (tt - time)) / (scene2.SceneTime * MILLS_TO_TIME);
                    if (joints.Length > joint)
                    {
	...
	bool SliderSlider(int idx) {
        float newJoint = GUI.HorizontalSlider(new Rect(400, 35 * (idx + 1), 200, 35), joints[idx],
            baseJointValues[idx] + jointOffsetRotations[idx] - 180,
             baseJointValues[idx] + jointOffsetRotations[idx] + 180);
