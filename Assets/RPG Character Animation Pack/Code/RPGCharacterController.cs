...
//		Aiming();
	...
	void OnTriggerEnter(Collider collide){
		...
		if(collide.gameObject.layer == 4){
			...
			animator.SetInteger("Weapon", 0);
			weapon = Weapon.UNARMED;
			StartCoroutine(_WeaponVisibility(leftWeapon, false));
			StartCoroutine(_WeaponVisibility(rightWeapon, false));
	...
	public IEnumerator _SwitchWeapon(int weaponNumber){
		...
		//If character is in Unarmed
		else if(weapon == Weapon.UNARMED){
			if(weaponNumber == -1){
				StartCoroutine(_SheathWeapon(-1, 0));
			}
			else{
				StartCoroutine(_UnSheathWeapon(weaponNumber));
			}
		}
		//character has 2 handed weapon or Unarmed
		else if(weapon != Weapon.ARMED && weapon != Weapon.ARMEDSHIELD && weapon != Weapon.SHIELD){
		...
		//character has 1 or 2 1hand weapons and/or shield
			...
			//character is switching to 2 hand weapon or unarmed or relax, put put away all weapons
			if((weaponNumber < 7 && weaponNumber > 17) || weaponNumber == 20 || weaponNumber < 1){
	...
	public IEnumerator _SheathWeapon(int weaponNumber, int weaponDraw){
		...
		//if switching to unarmed or relaxed, don't set "Armed" until after 2nd weapon sheath
		if(weaponDraw == 0 || weaponDraw == -1){
			if(leftWeapon == 0 && rightWeapon != 0){
		...
		//if switched to unarmed or relaxed
		if(leftWeapon == 0 && rightWeapon == 0){
			animator.SetInteger("LeftRight", 0);
			animator.SetBool("Armed", false);
			if(weaponNumber == -1){
		...
		if(instantWeaponSwitch){
			StartCoroutine(_WeaponVisibility(weaponNumber, false));
		}
		else{
			StartCoroutine(_WeaponVisibility(weaponNumber, false));
			if(weaponDraw == -1){
	...
	public IEnumerator _UnSheathWeapon(int weaponNumber){
		...
		//Switching from Unarmed or Relax
		if(weaponNumber == 0 || weaponNumber == -1){
			...
			if(!instantWeaponSwitch){
				...
				StartCoroutine(_WeaponVisibility(weaponNumber, true));
				animator.SetTrigger("InstantSwitchTrigger");
			}
			if(weaponNumber == 0){
				weapon = Weapon.UNARMED;
	...
	public IEnumerator _WeaponVisibility(int weaponNumber, bool visible){
