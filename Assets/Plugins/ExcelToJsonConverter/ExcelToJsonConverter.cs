...
{	
	...
	public void ConvertExcelFilesToJson(string inputPath, string outputPath, bool recentlyModifiedOnly = false, string sheetName = "", bool separateFilePerSheet = true)
	{
		...
		if (recentlyModifiedOnly && separateFilePerSheet)
		{
		...
		{
			if (!ConvertExcelFileToJson(excelFiles[i], outputPath, sheetName, separateFilePerSheet))
			{
				succeeded = false;
				break;
			}
	...
	/// <param name="outputPath">Output path.</param>
	public bool ConvertExcelFileToJson(string filePath, string outputPath, string sheetName = "", bool separateFilePerSheet = true)
	{
        ...
        if (separateFilePerSheet)
        {
            ...
            {
                if (!string.IsNullOrEmpty(sheetName) && !sheetName.Equals(excelData.Tables[i].TableName)) continue;

                spreadSheetJson = GetSpreadSheetJson(excelData, excelData.Tables[i].TableName);
                if (String.IsNullOrEmpty(spreadSheetJson))
                {
                    Debug.LogError("Excel To Json Converter: Failed to convert Spreadsheet '" + excelData.Tables[i].TableName + "' to json.");
                    return false;
                }
                else
                {
        ...
        {
            string excelFileJson = GetExcelJson(excelData, sheetName);
            if (String.IsNullOrEmpty(excelFileJson))
            {
                Debug.LogError("Excel To Json Converter: Failed to convert Excel file '" + excelData.DataSetName + "' to json.");
                return false;
            }
            else
            {
                // The file name is the sheet name with spaces removed
                string fileName = excelData.DataSetName;
                WriteTextToFile(excelFileJson, outputPath + "/" + fileName + ".txt");
                Debug.Log("Excel To Json Converter: " + excelData.DataSetName + " successfully written to file.");
            }
        }
	...
	private DataSet GetExcelDataSet(string filePath)
	{
		...
		DataSet data = new DataSet(Path.GetFileNameWithoutExtension(filePath));
	...
	private DataTable GetExcelSheetData(IExcelDataReader excelReader)
	{
		...
		while (excelReader.Read())
		{
			...
			for (int i = 0; i < excelReader.FieldCount; i++) 
			{
                ...
                else if (table.Columns.Count > i) // Otherwise, add as rows
                {
    ...
    /// <summary>
    /// Gets the json data for the specified DataSet.
    /// </summary>
    /// <returns>The spread sheet json.</returns>
    /// <param name="excelDataSet">Excel data set.</param>
    /// 
    private string GetExcelJson(DataSet excelDataSet, params string[] sheetNameFilters)
    {
        Dictionary<string, DataTable> dict = new Dictionary<string, DataTable>();

        for (int sheet = 0; sheet < excelDataSet.Tables.Count; sheet++)
        {
            DataTable dataTable = excelDataSet.Tables[sheet];

            if (sheetNameFilters.Length > 0 && Array.FindIndex(sheetNameFilters, (s) => { return string.IsNullOrEmpty(s) || s.Equals(dataTable.TableName); }) == -1)
                continue;
            ...
            dict[dataTable.TableName] = dataTable;
        }

        // Serialze the data table to json string
        return Newtonsoft.Json.JsonConvert.SerializeObject(dict, Formatting.Indented);
    }
	...
	private List<string> RemoveUnmodifiedFilesFromProcessList(List<string> excelFiles, string outputDirectory)
	{
		...
		for (int i = excelFiles.Count - 1; i >= 0; i--)
		{
			...
			for (int j = 0; j < sheetNames.Count; j++)
			{
				...
				string outputFile = outputDirectory + "/" + sheetNames[j] + ".txt";
