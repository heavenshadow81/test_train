...
namespace ML.T_Sports.HandBall
{
    ...
    public class HandBall_BallFire : MonoBehaviour
    {
        public Animation keeper_Aniamtion;
        public int key;
        public Camera cam;
        public Transform origin;
        public GameObject BallObj;
        public Ball_Ctrl_HandBall ballpre;

        public HandBall_Manager manager;
        ...
        void Update()
        {
            ...
            if (!keeper_Aniamtion.isPlaying)
            {
                keeper_Aniamtion.Play("Goal_Keeper_Idle");
                    ...
                    */                   
                    ...
                    if (xposition > 0 && xposition < 1920)
                    {
                        if (Physics.Raycast(ray, out hit, Mathf.Infinity, mask))
                        {
                            ...
                            Ball_Ctrl_HandBall ballPre = ball.GetComponent<Ball_Ctrl_HandBall>();
...
#region KeeperAnimation
        public void Keeper_AnimationPlay()
        {
            if (xposition < 750)
            {
                key = 4;
            ...
            {
                case 1:
                    keeper_Aniamtion["handball_R_down_block"].speed = 0.65f;

                    keeper_Aniamtion.Play("handball_R_down_block");

                    break;
                case 2:
                    keeper_Aniamtion["handball_L_down_block"].speed = 0.65f;

                    keeper_Aniamtion.Play("handball_L_down_block");
                    break;

                case 3:
                    keeper_Aniamtion["handball_R_up_block"].speed = 0.7f;

                    keeper_Aniamtion.Play("handball_R_up_block");
                    break;

                case 4:
                    keeper_Aniamtion["handball_L_up_block"].speed = 0.7f;

                    keeper_Aniamtion.Play("handball_L_up_block");
                    break;

                case 5:

                    keeper_Aniamtion.Play("Goal_Keeper_Upper");
                    break;
                default:
                    keeper_Aniamtion.Play("Goal_Keeper_Upper");
                    break;
